  ...
  <v-card rounded="lg" elevation="2" class="host-card h-100 transition-swing" variant="tonal">
    <div class="d-flex justify-space-between align-center px-4 pt-3">
      <div class="d-flex align-center text-truncate">
        <span class="font-weight-bold text-truncate" :title="hostData.domain">{{
          hostData.domain
        }}</span>
      </div>
    </div>
    ...
    <v-card-text class="pt-2 pb-4">
      <v-row no-gutters class="mb-2 align-center">
        ...
        <v-col cols="9">
          <v-chip
            :color="getBoolColor(hostData.using_cloudflare)"
            size="x-small"
            label
            variant="tonal"
    ...
    <v-divider></v-divider>
    <v-card-actions>
      <v-icon :color="hostData.meta.disabled ? 'grey' : 'success'">
