...
<template>
  ...
  >
    ...
    <template #item.value="{ item }">
      <v-chip v-for="ip in item.value" :key="ip" size="small" class="ma-1" variant="tonal">
        {{ ip }}
      </v-chip>
    </template>

    <template #item.using_cloudflare="{ item }">
      <v-chip :color="getBoolColor(item.using_cloudflare)" size="small" variant="tonal">
    ...
    <template #item.actions="{ item }">
      <v-menu min-width="120">
        ...
        <v-list density="compact">
          <v-list-item @click="$emit('edit', item.domain)">
            <template #prepend>
              <v-icon size="small" color="primary">mdi-file-edit-outline</v-icon>
            </template>
            <v-list-item-title>编辑</v-list-item-title>
          </v-list-item>
          <v-list-item @click="$emit('delete', item.domain)">
            <template #prepend>
