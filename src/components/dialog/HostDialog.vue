...
const newHost = ref<HostData>(
  props.isAdding ? { ...defaultHost } : structuredClone(toRaw(props.initialValue))
)
    ...
    <v-form ref="hostForm" @submit.prevent="saveHost">
      <v-card>
        ...
        <v-alert type="info" variant="tonal">
          支持<a href="https://wiki.metacubex.one/handbook/syntax/#_8" target="_blank"
            >域名通配符</a
          >
        </v-alert>
        <v-card-actions>
          ...
          <v-btn color="primary" type="submit" :loading="saveHostLoading">保存 </v-btn>
        </v-card-actions>
      </v-card>
    </v-form>
  </v-dialog>
